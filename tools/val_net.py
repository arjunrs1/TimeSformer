...
@torch.no_grad()
def perform_val(test_loader, model, test_meter, cfg, writer=None):
    """
    For classification:
...
def val(cfg):
    ...
    real_test_loader = loader.construct_loader(cfg, "test")
    ...
    test_meter = perform_val(test_loader, model, test_meter, cfg, writer)
    if writer is not None:
        writer.close()
